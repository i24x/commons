...
public class MargeProc {
    ...
    public void reader(FileMargeBusi[] margeFile, String outPath) {
        ...
        // 开始合并操作
        // 1,构建小顶堆操作`
        PriorityQueue<ByteHeadInfo> smallHeap = new PriorityQueue<>(margeFile.length, (o1, o2) -> {
        ...
        for (int i = 0; i < margeFile.length; i++) {
            if (margeFile[i].getBufferReadIndex() <= margeFile[i].getFileReadIndex()) {
                smallHeap.offer(new ByteHeadInfo(margeFile[i].getBuffer()[margeFile[i].getBufferReadIndex()], i));
                // 将索引向前推进一步
     ...
     *            小顶堆信息
     */
    private void readFileNextByte(FileMargeBusi margeFile, int index, PriorityQueue<ByteHeadInfo> smallHeap) {
        // 如果当前中的缓冲区还未读取完，从先从缓冲区中读取
        if (margeFile.getBufferReadIndex() < margeFile.getFileReadIndex()) {
            smallHeap.offer(new ByteHeadInfo(margeFile.getBuffer()[margeFile.getBufferReadIndex()], index));
            // 将索引向前推进一步
