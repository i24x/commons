...
public class SolutionStack {
    ...
    enum Oper {
        ADD('+', (i, j) -> i + j), MINUS('-', (i, j) -> i - j), MULTIPLICATION('*', (i, j) -> i * j),
        DIVISION('/', (i, j) -> i / j), LEFT('(', null), RIGHT(')', null);
    ...
    public int calculate(String s) {
        ...
        while (curIndex < dataChars.length) {
            ...
            // 操作符压栈操作
            else if (currItem == Oper.ADD.oper || Oper.MULTIPLICATION.oper == currItem
     ...
     * 进行栈的计算操作
     *
     * @param tmpCountValStack
     *            栈操作数
     * @param tmpCountOperStack
     *            栈数据操作符
     * @return 计算结果
     */
