 ...
 *
 * <p>
 * 加入首次访问的原因是：在首次时下标从0开始，而后，每次需要在当前下标上加1，以防止重复访问
 *
 * <p>
 * 使用更优雅的判重方式，即当当前不判断下，是否与前一个数相同，如果相同，后序列长的可处理。回塑的减枝操作
 *
 * @author liujun
 * @version 0.0.1
 * @date 2019/09/26
...
public class SolutionCombinationSum2ExistsOther {
     ...
     *            是否为首次访问
     */
    private void recurstion(int[] candidates, int target, int currvalue, int index, List<List<Integer>> result,
        List<Integer> currcollect, boolean first) {
