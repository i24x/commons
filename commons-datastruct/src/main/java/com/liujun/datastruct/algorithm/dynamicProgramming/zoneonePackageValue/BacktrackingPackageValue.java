...
public class BacktrackingPackageValue {
     ...
     *            最大重量限制
     */
    public void countMaxValue(int[] items, int[] values, int itemIndex, int sumWeight, int sumValue, int maxNum,
        int maxWeight) {
        ...
        if (sumWeight + items[itemIndex] < maxWeight) {
            countMaxValue(items, values, itemIndex + 1, sumWeight + items[itemIndex], sumValue + values[itemIndex],
                maxNum, maxWeight);
        }
    }
}
