...
public class UserDao {
    ...
    public UserDto getUserByUsername(String username) {
        ...
        // 连接数据库查询用户
        List<UserDto> list =
            jdbcTemplate.query(sql, new Object[] {username}, new BeanPropertyRowMapper<>(UserDto.class));
    ...
    public List<String> findPermissionsByUserId(String userId) {
        String sql =
            "SELECT * FROM t_permission WHERE id IN " + "(SELECT permission_id FROM t_role_permission WHERE role_id "
                + "IN (SELECT role_id FROM t_user_role WHERE user_id = ?))";
        List<PermissionDto> list =
            jdbcTemplate.query(sql, new Object[] {userId}, new BeanPropertyRowMapper<>(PermissionDto.class));
